                ...
                if ($stmt->prepare($query)) {
                    ...
                    if ($stmt->execute()) {
                        if ($_SESSION['update'] === "Not updated") {
                            ...
                            else echo "<p><output style=\"color: indianred;\">Статистика команды не была обновлена</output></p>";
                            ...
                            $query = "UPDATE `players` SET `team_id` = $_SESSION[update] WHERE id = $_POST[cap]";
                            if ($stmt->prepare($query) && $stmt->execute())
                                echo "<p><output style=\"color: seagreen;\">Данные игрока о команде были успешно обновлены</output></p>";
                            else {
                                ?>
            ...
            if ($stmt->prepare($query) && $stmt->execute()) {
                ...
                if ($stmt->num_rows == 0 && !isset($_POST['edit'])) {
                    ...
                    <div style="color: indianred;">
                        ...
                        <p><output>Для добавления команды, пожалуйста, добавьте нового игрока или обновите старого
                                так, чтобы он не состоял в какой-либо команде во вкладке
                                <output style="color: darkcyan">Информация -> Игроки</output> через кнопку обновления
                                игрока</output></p>
                        <p><output style="color: navy;">Напомним, что у команды всегда должен быть капитан, то есть если Вы хотите
                                добавить новую команду с капитаном из уже существующей команды, то в первую очередь
                                необходимо изменить капитана в существующей команде (или удалить её), обновить статус
                                команды у будующего капитана на "Не в команде" и только потом добавить новую команду</output></p>
                    </div>
                    <?php
                } else {
                    while ($stmt->fetch())
                        ...
                        if ($stmt->prepare($query)
                            ...
                            if ($stmt->num_rows == 0) {
                                ...
                                <div style="color: indianred;">
                                    <p><output>Нет игроков, которые могли бы стать капитанами</output></p>
                                    <p><output>Для добавления команды, пожалуйста, добавьте нового игрока или обновите старого
                                            так, чтобы он не состоял в какой-либо команде</output></p>
                                    <p><output style="color: navy;">Напомним, что у команды всегда должен быть капитан, то есть если Вы хотите
                                            добавить новую команду с капитаном из уже существующей команды, то в первую очередь
                                            необходимо изменить капитана в существующей команде (или удалить её), обновить статус
                                            команды у будующего капитана на "Не в команде" и только потом добавить новую команду</output></p>
                    ...
                    <form name="insertNewPlayground" method="POST" action="team.php">
                        <p><label>Название команды<input type="text" name="team_name" maxlength="50"
                                                         ...
                                                         pattern="[0-9A-ZА-Я^ЪЬ]{1}.*"
                                                         placeholder="Team Smith"/></label></p>
                        <p><label>Капитан<select name="cap">
                                    <option selected
                                            value="<?php echo $cap_id; ?>"><?php echo "$cap_name [$cap_id]"; ?></option>
