            ...
            if ( $stmt->prepare($query) ) {
                ...
                if ( $stmt->num_rows == 0 ) {
                    ...
                    <div style="color: indianred;">
                        ...
                        <p><output>Для добавления команды, пожалуйста, добавьте нового игрока или обновите старого так, чтобы он не состоял в какой-либо команде</output></p>
                        <p><output>Напомним, что у команды всегда должен быть капитан, то есть если Вы хотите добавить новую команду с капитаном из уже существующей команды, то в первую очередь необходимо изменить капитана в существующей команде (или удалить её), обнавить статус команды у будующего капитана на "Не в команде" и только потом добавить новую команду</output></p>
                    </div>
                    <?php
                }
                else {
                    ...
                    <form name="insertNewPlayground" method="POST" action="team.php">
                        <p><label>Название команды<input type="text" name="team_name" maxlength="50" pattern="[0-9A-ZА-Я^ЪЬ]{1}" placeholder="Team Smith"/></label></p>
