...
//                    echo $username;
        ...
        <form name="auth" method="POST" action="registration.php">
            <p><label>Имя<input type="text" name="name" maxlength="50" pattern="[A-ZА-Я^ЪЬ]{1}[A-ZА-ЯЁa-zа-яё -]{1,49}" placeholder="Adam" required /></label></p>
            <p><label>Фамилия<input type="text" name="surname" maxlength="100" pattern="[A-ZА-Я^ЪЬ]{1}[A-ZА-ЯЁa-zа-яё -]{1,99}" placeholder="Smith" required /></label></p>
            <p><label>Логин<input type="text" name="username" minlength="2" maxlength="10" placeholder="MyLogin" required /></label></p>
            <p data-tooltip="Пароль должен содержать заглавные и строчные символы, цифры и иметь длниу не менее 6 символов"><label>Пароль<input type="password" name="passwd" minlength="6" pattern=<?php echo $pattern; ?> placeholder="password123" required /></label></p>
